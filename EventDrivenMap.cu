...
void EventDrivenMap::SetParameters( const ParameterList* pParameterList)
{
  mNetworkSize = (*pParameterList).networkSize;
  mNoThreads   = (*pParameterList).noThreads;
  mNoBlocks    = (mNetworkSize+mNoThreads-1)/mNoThreads;
...
void EventDrivenMap::InitialiseNetwork()
{
  ...
  InitialiseNetworkKernel<<<mNoBlocks,mNoThreads>>>( mpGlobalState,
    mpGlobalZone, mpRefractTime, mNetworkSize);
  CUDA_CHECK_ERROR();
...
{
  CopyDataToPlotBufferKernel<<<mNoBlocks,mNoThreads>>>( mpPlotVarYHelper, mpGlobalState, mpGlobalZone, mpRefractTime, mNetworkSize);
  char str[50];
...
__global__ void CopyDataToPlotBufferKernel( float* pPlotVarY, const float4* pGlobalState, const unsigned int* pGlobalZone, const float* pRefractTime, const unsigned int networkSize)
{
  ...
  if (index<networkSize)
  {
    ...
    //pPlotVarY[index] = pRefractTime[index];
    //pPlotVarY[index] = (float) pGlobalZone[index];
  }
}
...
__device__ float FindSpikeTime( const float4 state, const float local_I)
{
  ...
  while (fabs(f)>tol*(iterate/1000)) {
    spikeTime -= f/df;
    f  = fun3( spikeTime, state.x, state.y, state.z, state.w, local_I, V_th);
    df = dfun3( spikeTime, state.x, state.y, state.z, state.w, local_I);
    printf("Iterate = %d,\t Index = %d,\t v = %f,\t zone = 3, \t time = %f\n",iterate,threadIdx.x,f,spikeTime);
    iterate++;
  }
